...
### Real secrets management (this pass)

- **New `app/secrets/` module**: a pluggable `SecretsProvider` — `EnvSecretsProvider` (reads `os.environ`, the existing behavior and default) or `VaultSecretsProvider` (HashiCorp Vault's KV v2 engine, `SECRETS_PROVIDER=vault`) selected once via `get_secret(key, default)`, the one function the rest of the app now calls for credentials. Vault mode expects this app's whole secret bundle as fields on one KV v2 document (`VAULT_SECRET_PATH`, default `apex-fintech`) — the same key names the `.env` file already uses — so turning it on doesn't require restructuring anything, just populating that document. The fetched document is cached in memory for `VAULT_CACHE_TTL_SECONDS` (default 300s) so a provider client constructed per-request doesn't cost a Vault round trip every time, while still picking up a rotated secret without an app restart; a Vault fetch failure serves the last-known-good cached value rather than taking down every provider-backed feature over one bad request.
- **Migrated every real credential this app reads** through `get_secret()`: `JWT_SECRET`, `DATABASE_URL` (commonly embeds credentials), and every provider API key/secret (`STRIPE_SECRET_KEY`, `DWOLLA_APP_KEY`/`DWOLLA_APP_SECRET`, `PLAID_CLIENT_ID`/`PLAID_SECRET`, `DIDIT_API_KEY`, `QBO_CLIENT_ID`/`QBO_CLIENT_SECRET`, `COMPLYADVANTAGE_API_KEY`, `WORKOS_API_KEY`/`WORKOS_CLIENT_ID`, `SMTP_USERNAME`/`SMTP_PASSWORD`, `EXCHANGE_RATE_API_KEY`, and the three webhook signing secrets). Left as plain `os.getenv()` on purpose: non-secret configuration — `USE_REAL_*` feature flags, base URLs, timeouts, ports — since a secrets manager should hold credentials, not app config.
- **Found and fixed a real, standing security bug while doing this**: `JWT_SECRET` — the key that signs every access token in the app — had a hardcoded fallback (`"supersecretkey123"`) with no warning if it was still in effect. Anyone who knew that string could forge a valid login for any user in any deployment that forgot to set the real one. Now: the fallback still exists (so local dev/tests need zero setup), but app startup logs a warning if it's in effect, escalating to an error-level log if any `USE_REAL_*` provider flag is also on — i.e. exactly the configuration that looks like it's headed to production while still signing tokens with a public, hardcoded string.
  - **Still needed for production:** a real Vault (or AWS/GCP Secrets Manager — not built this pass; Vault was chosen as the concrete first backend, but the `SecretsProvider` interface is provider-agnostic) instance and `VAULT_ADDR`/`VAULT_TOKEN`; short-lived Vault tokens with renewal instead of a long-lived static token; and secret rotation tooling (today, rotating a secret in Vault is picked up within `VAULT_CACHE_TTL_SECONDS`, but nothing automates the rotation itself).
...
- **Self-heals an already-running deployment automatically**: a database that already has this app's tables but no Alembic bookkeeping (e.g. a Neon database created before this change, back when `create_all()` populated it) gets `alembic stamp head` run against it instead of `upgrade` — startup detects "has tables, no `alembic_version`" and stamps rather than trying to recreate anything already there, then proceeds normally. No manual step needed. (This same situation is also how the test suite's `TestClient` fixtures touch the real dev `ramp_clone.db` as a startup-event side effect independent of each test's own isolated database — without this auto-heal, that would have broken every test using `with TestClient(app) as client:`.)
